...
function AddInsertDialog() {
  ...
  // the aspect follows the device unless the user picked another one
  const pickDevice = (next: DeviceType) => {
    setDevice(next)
    const preset = DEVICES.find((d) => d.id === next)
    if (preset) setAspect(preset.aspect)
  }
  ...
  async function submit() {
    ...
    try {
      await addInsert({
        ...
        promptEn: f.promptEn || undefined,
        shortPromptRu: f.shortPromptRu || undefined,
        shortPromptEn: f.shortPromptEn || undefined,
        negativePromptRu: f.negativePromptRu || undefined,
        negativePromptEn: f.negativePromptEn || undefined,
        technicalNotesRu: splitLines(f.technicalNotesRu),
        technicalNotesEn: splitLines(f.technicalNotesEn),
      })
    } catch {
      return
    }
        ...
        <div className="flex flex-col gap-4 py-2">
          ...
          <div className="grid gap-4 sm:grid-cols-4">
            ...
            <Fld label={t("library.device")}>
              <Select value={device} onValueChange={(v) => pickDevice(v as DeviceType)}>
                <SelectTrigger className={triggerCls}>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Fld label={t("editor.descriptionEn")} hint={t("editor.optional")}>
              <Textarea
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Fld label={t("editor.promptEn")} hint={t("editor.optional")}>
              <Textarea
                value={f.promptEn}
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Fld label={t("editor.shortPromptEn")} hint={t("editor.optional")}>
              <Textarea
                value={f.shortPromptEn}
                onChange={(e) => set("shortPromptEn")(e.target.value)}
                rows={2}
                className={areaCls}
              />
            </Fld>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Fld label={t("editor.negativePromptEn")} hint={t("editor.optional")}>
              <Textarea
                value={f.negativePromptEn}
                onChange={(e) => set("negativePromptEn")(e.target.value)}
                rows={2}
                className={areaCls}
              />
            </Fld>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <Fld label={t("editor.technicalNotes")} hint={t("editor.technicalNotesHint")}>
              <Textarea
                value={f.technicalNotesRu}
                onChange={(e) => set("technicalNotesRu")(e.target.value)}
                rows={3}
                className={areaCls}
              />
            </Fld>
            <Fld label={`${t("editor.technicalNotes")} (en)`} hint={t("editor.optional")}>
              <Textarea
                value={f.technicalNotesEn}
                onChange={(e) => set("technicalNotesEn")(e.target.value)}
                rows={3}
                className={areaCls}
              />
            </Fld>
