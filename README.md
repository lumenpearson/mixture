...
Проект деплоится на Vercel из этого репозитория: каждый push собирает превью, слияние в
`master` — продакшен. Корневой `vercel.json` описывает сборку монорепо (пресет Next.js,
`pnpm build`, вывод `apps/web/.next`), поэтому Root Directory в проекте не нужен; `pnpm
